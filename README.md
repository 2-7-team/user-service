...
유저 서비스의 로그인 및 회원가입 API는 permitAll로 설정하여 인증 없이 접근이 가능하도록 하였고,
로그인 성공 시 JWT 토큰을 발급합니다. 이후 클라이언트는 이 토큰을 모든 요청의 헤더에 포함시켜야 하며,
게이트웨이는 해당 토큰의 존재 여부를 확인하고,
유효한 경우 토큰을 파싱하여 userId와 role 정보를 추출합니다.
이 유저의 정보를 요청의 헤더에 추가하여 각 마이크로서비스로 전달되도록 설계했습니다.

각 마이크로서비스는 게이트웨이를 통해 전달받은 헤더의 userId와 role 정보를 기반으로 사용자 인증 및 권한 검증을 수행하며,
유저 서비스의 로그인 및 회원가입을 제외한 모든 API 요청은 해당 사용자 정보를 포함해야만 접근이 가능하도록 구현하였습니다.
</br>
